...
coverage: ## check code coverage quickly with the default Python
	coverage run --source atlas_api_sdk -m pytest
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/atlas_api_sdk.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ mongodb_atlas_api_sdk
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
conform	: ## Conform to a standard of coding syntax
	isort --profile black mongodb_atlas_api_sdk
	black mongodb_atlas_api_sdk tests
	find mongodb_atlas_api_sdk -name "*.json" -type f  -exec sed -i '1s/^\xEF\xBB\xBF//' {} +
